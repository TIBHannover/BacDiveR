...
returned to you. Use any of these methods:
...
extract_temps <- function(dataset, taxon_name) {
  ...
  unlist() %>%
  as.numeric() %>%
  tibble::tibble(temp_C = ., taxon = taxon_name)
}
...
## Another example: Extracting NCBI-IDs

Again, we need to look up the path of the data fields of interests, using one of the above-listed methods.
`NCBI_Gi` can be replaced by `seq_acc_num`.

```{r NCBI}
extract_NCBI_IDs <- function(dataset, taxon_name) {
  purrr::map(.x = dataset, .f = ~ .x$molecular_biology$sequence$NCBI_Gi) %>%
    unlist() %>%
    tibble::tibble(
      BacDive_ID = names(.),
      NCBI_Gi = .,
...
NCBI_IDs_Bac <- extract_NCBI_IDs(Bac_data, taxon_1)
NCBI_IDs_At <- extract_NCBI_IDs(At_data, taxon_2)
```
