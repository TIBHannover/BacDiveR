...
that type a plant pathogenic species. **Note the** results list with the 
"hits" on the right, and the now much longer **URL**. It contains/encodes all 
the terms and parameters of your advanced search.

[![Advanced search results](advanced-search-plant-pathogens.png)](https://bacdive.dsmz.de/advsearch?advsearch=search&site=advsearch&searchparams%5B5%5D%5Bsearchterm%5D=1&searchparams%5B156%5D%5Bsearchterm%5D=&searchparams%5B158%5D%5Bsearchterm%5D=1)
...
1. Paste the copied URL into a call to the `data <- retrieve_search_results("…")` function.

1. Enjoy the list of downloaded datasets, just as you would after using
`data <- retrieve_data(searchTerm = ..., searchType = ...)`.
